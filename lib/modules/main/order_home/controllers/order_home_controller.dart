...
class OrderHomeController extends BaseController {
  ...
  Future<void> showOptProduct(
      {required ProductRequest item, required int index}) async {
    await BottomSheetUtil.showBottomSheet(
      child: WidgetMenuOption(
        title: 'Tuỳ chọn'.tr,
        option: [
          MenuOptionModel(
              onAction: () {
  ...
  Future<void> onEditOrderProduct(ProductRequest? item) async {
    await Get.toNamed(Routes.ORDER_PRODUCT, arguments: item)?.then((value) {
      if (value != null && value is ProductRequest) {
        if (value.index != null) {
          products[value.index!] = value;
        } else {
  ...
  Future<void> onRemoveProduct(int index) async {
    await BottomSheetUtil.createConfirmBottomSheet(
      title: 'Xác nhận xoá sản phẩm'.tr,
      message: ''.tr,
      confirmTitle: 'ok'.tr,
      onConfirm: () {
        removeProduct(index);
